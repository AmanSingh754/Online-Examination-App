...
function BdeLogin() {
  useBodyClass("landing-page bde-landing auth-page");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (role !== "BDE") {
        setError("This login is for BDE accounts only.");
        setIsLoggingIn(false);
        return;
      }
      ...
      console.error("BDE login error:", err);
    ...
    <div className="landing-shell">
      <header className="landing-header">
        ...
        <nav className="landing-nav">
          <a href="/bde" className="nav-link">BDE Home</a>
          <a href="#" className="nav-link">Support</a>
          <a className="nav-cta" href="/bde/login">Login</a>
        </nav>
      </header>
      <main className="auth-main">
        <section className="auth-copy">
          <p className="hero-eyebrow">Business Executive Access</p>
          <h1>Track you registrations through your Dashboard.</h1>
        ...
        <section className="auth-card">
          <h2>BDE Login</h2>
          <p className="auth-meta">Use your BDE ID or email and password.</p>
          <form autoComplete="off" onSubmit={handleSubmit}>
            <input
              type="text"
              placeholder="BDE ID or Email"
              value={email}
